...
\section{Pumps}
\todo{talk more about the pumps used, because the only reason we use them is: "they are present in the setup", e.g. we don't care why other pumps are not as good. [debatable]}
...
\todo{"proven true for all cases"? that can't be correct for an estimate}
...
\section{Valves}
Valves can control the resistance introduced, by changing the diameter of the pipeline at a given point. \todo{re-write valves, this reads terrible}
...
\todo{there is a better word for handle in this context}

\section{Sensors}
To be able to monitor the system closely, different sensors are used.
\todo{write about the sensors used in the system}

\subsection{Flow Meter}
\subsection{$\Delta$Pressure Sensor}
\subsection{Power Sensor}
\todo{complete this list of subsections}
\todo{maybe take out of ToC? (subsection*)}

\todo{Do we want a section about xPC and Simulink Realtime?}
